...
|  ③ OS の資格情報マネージャーを利用  | Windows OS の資格情報マネージャーに資格情報を登録しておき、スクリプトから取得。| スクリプトの中に資格情報を記載せず、スクリプトと資格情報を分けて管理ができる。OS の UI からも更新・バックアップが可能。ID/パスワードをまとめて管理可能。 | OS 環境が乗っ取られた場合には、攻撃者に抜き出されやすい。 |
|  ④ Azure Automation の資格情報管理機能を利用  | Azure Automation を使った PowerShell スクリプトの定期実行の場合、Automation アカウントに登録しておいた資格情報を呼び出して利用可能。 | スクリプトの中に資格情報を記載せず、スクリプトと資格情報を分けて管理ができる。Azure の UI からも更新が可能。ID/パスワードをまとめて管理可能。 | Azure Automation 環境でのスクリプト実行でのみ利用可能。Azure 環境が乗っ取られた場合には、攻撃者に抜き出されやすい。 |
