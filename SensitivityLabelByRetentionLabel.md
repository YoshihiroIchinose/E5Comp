# 保持ラベルを利用した秘密度ラベルの適用
本ページでは、SharePoint Online および OneDrive for Business において、ライブラリ・フォルダ・ファイル単位の保持ラベル付けを行いつつ、保持ラベルを条件とした自動の秘密度ラベル付けを行う方法を紹介します。実環境への広範な展開においては、十分な検証を行った上で、実施ください。
