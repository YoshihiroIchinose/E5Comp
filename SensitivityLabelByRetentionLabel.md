# 保持ラベルを利用した秘密度ラベルの適用
本ページでは、SharePoint Online および OneDrive for Business において、ライブラリ・フォルダ・ファイル単位の保持ラベル付けを行いつつ、保持ラベルを条件とした自動の秘密度ラベル付けを行う方法を紹介します。実環境への広範な展開においては、保持ラベル・秘密度ラベルの十分な設計と検証を行った上で、実施ください。
...
<img src="https://github.com/YoshihiroIchinose/E5Comp/blob/main/img/RL_File02.png">
...
### 保持ラベルの定義と発行
Purview ポータルのデータ ライフサイクル管理のページに行き、保持ラベルとして以下の 4 つを定義します。この例では、秘密度ラベルを付与することを前提に、保持ラベルに想定する秘密度ラベルの名称を組み入れたものとしています。保持ラベルの使い方として、どれくらいの期間で削除するかの選択肢を与えるため、1年、3年、無期限の種類を用意しています。ユーザー側でコントロールしやすいように、期間が来て削除対象となったファイルは、「1か月で削除」の保持ラベルに切り替わるようにしていて、削除をキャンセルしたい場合には、個別に保持ラベルを「機密情報・3年」などに変更することで、対応できるようにしています。なお、この設定では、明示的な保持は行っていないため、ユーザー自身による削除で、一定期間経過後にファイルは消失します。秘密度ラベル付けだけを行いたい場合、保持ラベルのアクセションは設定してなくても OK です。
