...
SharePoint Online のファイルに Graph API を利用して秘密度ラベルを付与するサンプルです。権限付与時の組織の同意部分を除いて、ユーザー権限で動作するものです。
...
4. 管理の API のアクセス許可で、アプリケーションの許可として Sites.Selected の権限を与える
5. 管理の証明書とシークレットから Client Secret を新規作成して値をコピーしておく
